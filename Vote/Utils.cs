...
namespace Vote
{
	internal static class Utils
	{
		...
		internal static void Vote(CommandArgs args)
		{
			...
			if (vote.Opponents.Union(vote.Proponents).Contains(args.Player.Name))
			{
				data.AwaitingVote = true;
		...
		internal static void Confirm(CommandArgs args)
		{
			...
			if (data.Support)
			{
				data.OngoingVote.Proponents.Add(args.Player.Name);
			}
			else
			{
				data.OngoingVote.Opponents.Add(args.Player.Name);
			}
			...
			if (!VotePlugin.Config.ShowResult)
				...
				TSPlayer.All.SendInfoMessage("{0} voted {1} {2}!",
					...
					TShock.Utils.ColorTag(data.OngoingVote.ToString(), Color.SkyBlue));
			TShock.Log.ConsoleInfo($"{args.Player.Name} voted {(data.Support ? "for" : "against")} {data.OngoingVote}!");
		...
		internal static void OnVoteTimerElasped(Vote vote)
		{
			foreach (var player in TShock.Players.Where(p => p?.Active == true))
			{
				...
				else if (vote.Opponents.Union(vote.Proponents).All(ply => !player.Name.Equals(ply, StringComparison.Ordinal)))
				{
					vote.Neutrals.Add(player.Name);
